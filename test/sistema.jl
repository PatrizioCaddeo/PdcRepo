...
function x(t) 
	t_modulo = FLoat64 # t_modulo è il valore di t riscalato
	if(t>=0)
		t_modulo = t % T
	else
		t_modulo = (t % T) + T
	end
	n = floor(Int64, t/(T/N) ) # n è l'indice di approssimazione
	if floor n==0
		return a[1]
	elseif n==N
		return a[N+1]
	else
		return a[n+1]+(a[n+2]-a[n+1])/(T/N)*(t-(n+1)T/N)
	end
end
